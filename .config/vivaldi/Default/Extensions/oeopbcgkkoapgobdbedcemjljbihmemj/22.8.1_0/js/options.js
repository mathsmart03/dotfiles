		...
		if (tabName == "welcome") {
            ...
            async function initRecycleNotifs() {
                if (await storage.get("showNotificationDuration") == "infinite") {
                    $("#recycleNotification").show();
                } else {
                    $("#recycleNotification").hide();
                }
            ...
            $("#showNotificationDuration_rich paper-item").click(async function () {
                await waitForStorageSync();
                initRecycleNotifs();
            });
            
            $("#moveIntoActionCenter").click(async function() {
                if (this.checked) {
                    ...
                    if (await storage.get("notificationWakesUpScreenTemporarily")) {
                        ...
                        $("#notificationWakesUpScreenTemporarily")[0].checked = false;
                    }
                    $("#rich-close-after-x paper-item[value='infinite']").trigger("click");
                }
            });

            // refer to https://jasonsavard.com/forum/discussion/comment/28786#Comment_28786
            $("#notificationWakesUpScreenTemporarily").click(async function() {
                if (this.checked && await storage.get("moveIntoActionCenter")) {
                    ...
                    $("#moveIntoActionCenter")[0].checked = false;
                }
            });
			...
			$("#showOnlyRecentUnreadEmails").change(async function () {
                await waitForStorageSync();
			...
			$("#console_messages").change(async () => {
                await waitForStorageSync();
                await niceAlert("Click OK to restart the extension");
                reloadExtension();
            });

            $("#cacheStorage")[0].checked = localStorage["cacheIndexedDB"] != "false";

            $("#cacheStorage").change(function() {
                if (this.checked) {
            ...
            $("#disableAnalytics")[0].checked = localStorage["disableAnalytics"];

            $("#disableAnalytics").change(function() {
                if (this.checked) {
            ...
            $("#fetchOtherContacts").click(async () => {
                await niceAlert("Click OK to restart the extension");
                reloadExtension();
            });
...
async function loadAccountsOptions(loadAccountsParams = {}) {
    ...
    if (t) {
        ...
        setTimeout(async function() {
            ...
            $("#grant-host-permission").off().on("click", () => {
