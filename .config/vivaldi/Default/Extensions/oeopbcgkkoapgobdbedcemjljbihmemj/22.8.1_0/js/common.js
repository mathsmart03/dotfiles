...
class DetectClientClass {
    ...
    isChrome() {
        return this.findBrand("Google Chrome") || this.isChromium();
    }
    ...
    async getChromeChannel() {
        ...
        if (this.isChrome()) {
            ...
            for (let a = 0; a < data.length; a++) {
                ...
                if (osMatched) {
                    const browserVersion = (await navigator.userAgentData.getHighEntropyValues(["uaFullVersion"])).uaFullVersion;
                    ...
                    for (var b = 0; b < thisData.versions.length; b++) {
                        ...
                        if (browserVersion == thisVersion.previous_version
                        || browserVersion == thisVersion.version) {
	...
	if (globalThis.polymerPromise2?.then) {
		polymerPromise2.then(() => {
			if (globalThis.showError) {
                ...
                if (errorStr.name == "DBError" || errorStr.toLowerCase().includes("indexeddb")) {
                    showError(errorStr, {
                        text: "See solution",
                        onClick: () => {
