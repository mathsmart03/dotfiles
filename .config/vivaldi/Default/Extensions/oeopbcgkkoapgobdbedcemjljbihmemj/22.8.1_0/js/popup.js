...
function initOpenEmailEventListeners() {
	...
	$("#markAsRead, #markAsUnread").click(async function() {
		const mail = getOpenEmail();
		const $mail = getMailNode(mail);
	...
	$("#openEmailInBrowser").on("mouseup", event => {
		const mail = getOpenEmail();
			...
			if (mail.messages.last()) {
				...
				if (markAsReadSetting) {
                    ...
                    if (await mail.hasLabel(SYSTEM_SPAM)) {
                        $("#markAsNotSpam").unhide();
                    } else {
                        $("#markAsNotSpam").hidden();
                    }
				...
				$replyArea.find("#send, #sendAndArchive, #sendAndDelete").off().click(function(e) {
                    ...
                    const previousSendButtonHTML = $sendButtonClicked[0].innerHTML;
                    ...
                    showToast({toastId:"undoToast", duration:999, text: `${getMessage("sending")}...`, actionParams:{
                            onClick: function() {
                                ...
                                $sendButtonClicked[0].innerHTML = previousSendButtonHTML;
                            }
                        }
                    });
	...
	if (await storage.get("usedMarkAllAsReadButton")) {
		...
		openGenericDialog({
			...
			if (response == "ok") {
				...
				$account.find(".markAllAsReadButton").trigger("mouseup");
			}
		});
	}
}
    ...
    await asyncForEach(accounts, async (account, accountIndex, thisArray) => {
		...
		if (accountIndex != 0 && accountIndex == (thisArray.length-1) && !account.hasBeenIdentified()) {
			...
			$account.find(".accountOptionsMenuButton")
				.one("mouseup", async function() { // MUST use .one because mousedown will also be called when menu items *inside the dropdown all are also clicked
					
                    $(this).closest(".accountHeader").removeClass("sticky");

                    // patch for dark them when using account dropdown
                    if (window.darkTheme) {
                        $(".accountHeader").css("filter", "initial");
                        $(".accountHeader iron-image").css("filter", "invert(100%)");
                    }

                    // patch for reveal background image blur when using account dropdown
                    if (window.revealImage) {
                        $(".account").css("backdrop-filter", "initial");
                    }
	...
	if ($mail.length) {
		...
		if ($inboxMailActionButtonsTemplate.length) {
			...
			$mail.find(".markAsSpam")
				.attr("title", getMessage("reportSpam"))
				.on("mouseup", event => {
			...
			$mail.find(".markAsNotSpam")
				.attr("title", getMessage("notSpam"))
				.on("mouseup", event => {
                    executeMailActionAndHide(mail, "markAsNotSpam");
					return false;
			    })
            ;
...
async function init() {
    ...
    new Promise(async (resolve, reject) => {
		...
		if (fromToolbar && zoomFactor > 1) {
            $body.css({
                "width": MAX_POPUP_WIDTH / zoomFactor,
                "height": MAX_POPUP_HEIGHT / zoomFactor
            });
		}
    });
	...
	$body
		...
		.keydown(function(e) {
			...
			if (isFocusOnInputElement()) {
				...
				const $selectedMail = $(".mail").first();
				
				initInboxMailActionButtons($selectedMail);
				
				if (e.key == 'c' && !isCtrlPressed(e)) {
					$(".account:first .compose").trigger("mouseup");
						...
						if ($selectedMail.length) { // found unread email so open the email
							if (e.key == "Enter") {
								...
								if (isEmailView()) {
									...
									$selectedMail.trigger("mouseup");
								}
							} else {
								$selectedMail.find(".openMail").trigger("mouseup");
							}
					...
					if (isEmailView()) {
						...
						$selectedMail.find(".delete").trigger("mouseup");
					}
				} else if (e.key == 'e') { // archive
					if (isEmailView()) {
						$("#archive").click();
					} else {
						$selectedMail.find(".archive").trigger("mouseup");
					}
				} else if (e.key == '!') { // spam
					if (isEmailView()) {
						$("#markAsSpam").click();
					} else {
						$selectedMail.find(".markAsSpam").trigger("mouseup");
					}
				} else if (e.key == 's') { // star
					if (isEmailView()) {
						$("#openEmail .star").trigger("mouseup");
					} else {
						$selectedMail.find(".star").trigger("mouseup");
					}
				...
				// r = reply (if setting set for this)
					...
					if (isEmailView()) {
						...
						$selectedMail.find(".markAsRead").trigger("mouseup");
                    }
                } else if (e.key == "?") {
                    maxHeightOfPopup();
	...
	$(".share-button").on("mouseup", function() {
		maxHeightOfPopup();
	});
	...
	$("#maximize").mouseup(async function(e) {
		if (isCtrlPressed(e)) {
	...
	$("#mainOptions").on("mouseup", function() {
		maxHeightOfPopup();
	});
