    ...
    return new Promise((resolve, reject) => {
      this.xhr.onload = () => {
        if (this.xhr.status >= 400) {
          ...
          var response = { data: this.xhr.response }
          if (this.xhr.getResponseHeader("content-disposition")) {
            response.headers = {
              "content-disposition": this.xhr.getResponseHeader("content-disposition"),
            }
          }
          resolve(response)
        }
