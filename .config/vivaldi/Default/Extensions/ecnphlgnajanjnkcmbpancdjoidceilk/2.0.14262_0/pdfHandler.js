...
var EXTENSION_IDS = [
  ...
  'nlnndloljhkmcbihclnpkhpbpbapdmpc', // test
];
...
function getViewerURL(details, handlerInfo) {
  ...
  if (details) {
    ...
    if (cd) {
      var cdp;
      try {
        // parse() can throw an exception if header is invalid. If thrown, we bypass content disposition parsing
        cdp = contentDisposition.parse(cd);
      } catch(e) {
        // do nothing
      }
